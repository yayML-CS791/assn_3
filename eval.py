...
def parse_args():
    ...
    p.add_argument("--hf-token", type=str, default="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")
...
def main():
    ...
    trigam_counts_file = os.path.join(args.counts_dir, "trigram_counts.json")
    if not os.path.exists(cache_file):
        print(f"Warning: trigram_probs.pkl not found at {cache_file}")
        print("Please run: python precompute_trigram_probs.py first")
        sys.exit(1)
    
    reward_calc = FastRewardCalculator(cache_file, trigam_counts_file)
